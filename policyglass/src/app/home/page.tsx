...
// Define types for animation content
...
type AnimationContent = TextAnimation | FlagsAnimation | ReviewAnimation;

// Animation content data
const animationContent: AnimationContent[] = [
  {
    id: 1,
    type: "text",
    content: "Review these terms and conditions"
  },
  {
    id: 2,
    type: "flags",
  ...
  },
  {
    ...
    content: {
      ...
      rating: 5,
      text: "This tool helped me understand complex policy documents in minutes. The insights were clear and actionable. Highly recommended!",
...
export default function Home() {
  ...
  const [currentAnimation, setCurrentAnimation] = useState(0); // Index of current animation
  ...
  useEffect(() => {
    if (currentAnimation >= animationContent.length) {
      // All animations complete, navigate to results after a delay
      const navigateTimer = setTimeout(() => {
        router.push("/results");
      }, 2000); // Show the final animation for 2 seconds before navigating
      
      return () => clearTimeout(navigateTimer);
    }
    
    // For all animations except the last one, progress to the next after a delay
    if (currentAnimation < animationContent.length - 1) {
                ...
                <div className="absolute top-[20%] left-[5%] w-1/2 z-10">
                  <AnimatePresence>
                    {currentAnimation < animationContent.length && (
                      <motion.div
                        key={animationContent[currentAnimation].id}
                      ...
                      >
                        {animationContent[currentAnimation].type === "text" && (
                          <div className="text-center p-8">
                            <p className="text-2xl font-bold text-foreground bg-background bg-opacity-80 p-4 rounded-lg">
                              {animationContent[currentAnimation].content}
                            </p>
                          </div>
                        )}
                        
                        {animationContent[currentAnimation].type === "flags" && (
                          <div className="bg-background bg-opacity-80 backdrop-blur-sm rounded-lg shadow-lg p-6">
                            <h2 className="text-xl font-semibold text-foreground mb-4 text-center">
                              {animationContent[currentAnimation].content.title}
                            </h2>
                            ...
                            <div className="mb-4">
                              ...
                              <div className="space-y-2">
                                {animationContent[currentAnimation].content.flags.map((flag, index) => (
                                  <div key={index} className="flex items-start p-2 bg-red-100 rounded">
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {animationContent[currentAnimation].content.warnings.map((warning, index) => (
                                  <div key={index} className="flex items-start p-2 bg-yellow-100 rounded">
                        ...
                        {animationContent[currentAnimation].type === "review" && (
                          <div className="bg-background bg-opacity-80 backdrop-blur-sm rounded-lg shadow-lg p-8 text-center">
                            <h2 className="text-xl font-semibold text-foreground mb-4">
                              {animationContent[currentAnimation].content.title}
                            </h2>
                            <div className="flex justify-center mb-4">
                              {/* Star rating */}
                              {[...Array(animationContent[currentAnimation].content.rating)].map((_, i) => (
                            ...
                            <p className="text-foreground">
                              {animationContent[currentAnimation].content.text}
                            </p>
                            <p className="text-foreground font-medium mt-4">
                              - {animationContent[currentAnimation].content.author}
                            </p>
                          </div>
                        )}
                      </motion.div>
